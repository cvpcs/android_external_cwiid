...
{
	wminput_conf->plugins[id].rpt_mode_flags = flags;

	if (!(*wminput_init)) {
		unsigned char rpt_mode_flags;
		int i;

		rpt_mode_flags = wminput_conf->rpt_mode_flags;

		for (i=0; (i < CONF_MAX_PLUGINS) && wminput_conf->plugins[i].name; i++) {
			rpt_mode_flags |= wminput_conf->plugins[i].rpt_mode_flags;
		}
...
void wmplugin_set_conf(void *conf)
{
	wminput_conf = (struct conf *)conf;
}

void wmplugin_set_init(void *init)
{
	wminput_init = (char *)init;
}
